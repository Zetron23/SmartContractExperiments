...
contract SupplyChain {
    ...
    function purchaseFromCustomer(uint256 _tokenId, address _newOwner) external {
        require(_tokenId < tokenIdCounter, "Token does not exist.");
        require(tokens[_tokenId].state == State.Purchased, "Token is not available for customer purchase.");
        require(isTokenOwnedBy(msg.sender, _tokenId), "Caller does not own the token.");

        transferToken(_newOwner, _tokenId, State.Purchased);
    }

    function isTokenOwnedBy(address _owner, uint256 _tokenId) private view returns (bool) {
        uint256 length = ownedTokens[_owner].length;
        for (uint256 i = 0; i < length; i++) {
            if (ownedTokens[_owner][i] == _tokenId) {
                return true;
